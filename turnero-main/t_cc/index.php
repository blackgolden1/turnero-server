...
if (!isset($_SESSION)) {
	...
	// Establecer el conjunto de caracteres y collation
	mysqli_set_charset($turnos, "utf8");
	
	// Realizar consultas SQL
	
	// Consulta para obtener NODESERVER y NODEPORT
	$query_RsParams = "
		SELECT A.PARAVALOR AS NODESERVER,
			   (SELECT A2.PARAVALOR
				  FROM params_app A2 
				 WHERE A2.PARACODI = '2') AS NODEPORT
		  FROM params_app A 
		 WHERE A.PARACODI = '1'
	";
	...
	if (!$RsParams) {
		die("Query failed: " . mysqli_error($turnos));
	}
	...
	// Consulta para obtener MODUID y MODUNOMB
	$query_RsModulo = "
		SELECT MODUID AS CODIGO,
			   MODUNOMB AS NOMBRE
		  FROM modulos 
		 WHERE MODUTIPO = '1'
	";
	...
	// Procesar los resultados de $row_RsModulo según sea necesario
